...
const readFile = (url: string): any => {
    return fs.readFileSync(url);
}

const sendFile = (res: any, url: string): void => {

    let typeName: string | undefined = url.split(`.`)[1];
    ...
    const nowType: string = (type as any)[typeName];
    const filePath: string = typeName === `html`? `${path}${url}` : `${path}${nowPath}${url}`;
    console.log(url);
